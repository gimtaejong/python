...
def startscreen():
    start_background = pygame.image.load(os.path.join(image_path, "start.png"))
...
def main():        
    ...
    #먹어야 하는 것
    bad = pygame.image.load(os.path.join(image_path, "bad"+bad_num+".png"))
    ...
    #안먹어도 되는 것
    good = pygame.image.load(os.path.join(image_path, "good"+good_num+".png"))
    ...
    #라이프 회복
    toothbrush = pygame.image.load(os.path.join(image_path, "toothbrush.png"))
    ...
    #라이프
    life_num = 5
    life = pygame.image.load(os.path.join(image_path, "life"+str(life_num)+".png"))
    ...
    while running:
        ...
        if life_num == 0:
            ...
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_r: #r 입력 (재시작)
                        main()
                    else: #게임 종료
                        running = False
